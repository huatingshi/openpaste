...
启动页在命令下方留一行空白，显示青绿色、5 行高的 `OPENPASTE` 实心像素字标，以及当前快捷键、字符间隔和多行设置；窗口较窄或较矮时会使用单行 Logo。

**输入 `/` 会自动展开命令列表，继续输入可以筛选。** 例如 `/sp` 会筛选出 `/speed`。

- **↑ / ↓**：选择命令，列表超出可见区域时跟随选择滚动。
- **Tab**：补全选中命令，不执行。
- **Enter**：确认。没有参数的命令立即执行；需要参数的命令先补全，例如 `/speed `，输入数值后再回车。
- **Esc**：收起菜单，保留当前输入。发送文字时仍用于取消本次输入。
- **← / →、Home / End、Backspace / Delete**：编辑输入行。

菜单在原位置更新，收起后清除；调整窗口尺寸时会重新排版。输入或输出被重定向时使用普通文本，不输出颜色或光标控制字符。
...
终端界面测试覆盖自动展开、筛选、键盘选择、参数补全、长输入行、Unicode 字宽和窄窗口布局。
...
该测试创建独立的隐藏测试终端，验证 Ctrl+C 退出及快捷键释放、关闭终端后进程结束、真实菜单输入、缩放布局及 PowerShell 启动入口。
菜单测试从测试终端缓冲区读取画面，检查菜单清除与设置反馈，同时保存文本快照。
